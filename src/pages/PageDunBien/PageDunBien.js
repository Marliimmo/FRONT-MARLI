...
const getImageUrl = (url) => {
  ...
  const cleanUrl = url.startsWith('imagesBienMarli/') ? url : `imagesBienMarli/${url}`;
  return `https://marli-backend.onrender.com/bien/images/${cleanUrl}`;
};
  ...
  useEffect(() => {
    ...
    try {
      ...
      const fecthBienDisponible = async () => {
        const response = await fetch(
          `${process.env.REACT_APP_API_URL}/bien/all-biens?status=disponible&triPar=croissant&pageSize=5`,
        )
        if (response.ok) {
          const result = await response.json()
